        ...
        if (oldData == null) {
            ...
            if (BBsentialConnection.isCommandSafe(data.bbcommand)) {
                ...
                if (showChChest(data.getChests().getFirst().items)) {
                    ...
                    tellrawText = tellrawText.replace("@item", StringEscapeUtils.escapeJson(data.getChests().get(0).items.stream()
                            .map(ChChestItem::getDisplayName)
                            .toList()
                            .toString()));
                    tellrawText = tellrawText.replace("@coords", StringEscapeUtils.escapeJson(data.getChests().get(0).coords.toString()));
