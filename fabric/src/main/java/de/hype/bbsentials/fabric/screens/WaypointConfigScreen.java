...
public class WaypointConfigScreen {
    public static Screen create(Screen parent, WaypointData data) {
        ...
        main.addEntry(entryBuilder.startIntField(Text.literal("Y:"), data.position.y).setDefaultValue(data.position.y).setSaveConsumer((newy) -> data.position.x = newy).build());
        main.addEntry(entryBuilder.startIntField(Text.literal("Z:"), data.position.z).setDefaultValue(data.position.z).setSaveConsumer((newz) -> data.position.x = newz).build());
