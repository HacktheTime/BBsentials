...
public class NumCode {
    ...
    public void execute() {
        if (!(requiredPermission == null || (BingoNet.generalConfig.hasBBRoles(requiredPermission)))) {
        ...
        if (!commands.isEmpty()) {
            for (int i = 0; i < commands.size(); i++) {
                ...
                if (command.startsWith("\\")) {
                    ...
                    BingoNet.sender.addHiddenSendTask(commands.get(i), commandDelay.get(i));
            }
        } else {
            if (toPerform == null) {
            ...
            try {
                BingoNet.executionService.execute(toPerform);
            } catch (Exception e) {
