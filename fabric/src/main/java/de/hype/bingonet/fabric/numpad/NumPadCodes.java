...
import de.hype.bingonet.fabric.DebugThread;
import de.hype.bingonet.fabric.FabricTextUtils;
import de.hype.bingonet.fabric.screens.RoutesConfigScreen;
import de.hype.bingonet.fabric.screens.TrustedPartyMembersConfigScreen;
...
import static de.hype.bingonet.environment.packetconfig.PacketUtils.gson;
...
public class NumPadCodes {
    ...
    public NumPadCodes() {
        ...
        KeyBinding enterKey = new KeyBinding("Enter", GLFW.GLFW_KEY_KP_ENTER, "bingonet.numpad");
        KeyBinding clear = new KeyBinding("Clear", GLFW.GLFW_KEY_KP_ADD, "bingonet.numpad");
        KeyBinding remove = new KeyBinding("Remove", GLFW.GLFW_KEY_KP_DECIMAL, "bingonet.numpad");
        Thread t = new Thread(() -> {
            try {
                while (true) {
    ...
    public void addDefaultCodes(boolean all) {
        ...
        defaultCodes.add((new NumCode("0", Formatting.DARK_BLUE, BBRole.DEBUG, () -> BingoNet.executionService.execute(() -> ((DebugThread) EnvironmentCore.debug).onNumpadCode()))));
        defaultCodes.add((new NumCode("0", Formatting.DARK_BLUE, BBRole.DEBUG, () -> BingoNet.executionService.execute(() -> ((DebugThread) EnvironmentCore.debug).onNumpadCode()))));
    ...
    public void saveNumCodesToFile() {
        ...
        try (Writer writer = new FileWriter(new File(EnvironmentCore.utils.getConfigPath(), "BingoNet_Numpad_codes.json"))) {
    ...
    public void loadNumCodesFromFile() {
        File file = new File(EnvironmentCore.utils.getConfigPath(), "BingoNet_Numpad_codes.json");
